...
const mdLinks = (path, options = {}) => {
  ...
  return new Promise((resolve, reject) => {
    if (options.validate) {
      getLinks(pathAbs).then(arr => validateLink(arr)).then(response => resolve(response)).catch(console.error);
    } else {
      getLinks(pathAbs).then(response => resolve(response)).catch(console.error);
    }
  });
};
